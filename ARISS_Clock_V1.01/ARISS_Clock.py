...
def make_config_file():
    ...
    config_text = ['# ARISS Clock Config File',
                   ...
                   '#',
                   '# Verify this config file version number matches the',
                   '# ARISS Clock version. If not, delete the config file',
                   '# and restart ARISS Clock. A new config file will be',
                   ...
                   '#',
                   '# School local time zone with respect to UTC in this exact',
                   '# format only:.',
                   '#   STZ,HH where HH is hours offset from UTC.',
                   '# For example:',
                   '#   STZ,-5',
                   '#   STZ,-2.5',
                   '#',
                   '# Predicted ISS AOS and LOS times are in local ground station',
                   '# time in this exact format only:',
                   ...
                   '#',
                   'STZ,-5',
                   'AOS,2023-08-20 12:00:00',
                   'LOS,2023-08-20 12:10:00']
    # Create config file.
...
def time_school():
    ...
    """
    global stz
    utc_datetime = datetime.utcnow() + timedelta(hours=stz)  # Get time and add UTC offset from config file.
...
def time_aos():
    ...
    TODO (AOS Time): None.
    """
    delta = aos - time.time() + 1  # Difference between current local time and AOS time.
...
def time_los():
    ...
    TODO (LOS Time): None.
    """
    delta1 = aos - time.time() + 1  # AOS trigger to start ET.
    delta2 = los - time.time() + 1  # Difference between current local time and LOS time.
...
def time_pass_elapsed():
    ...
    """
    delta1 = aos - time.time() + 1  # AOS trigger to start ET.
    delta2 = los - time.time() + 1  # LOS trigger to stop ET.
...
try:
    ...
    AOS = tuple([int(x) for x in AOS[:10].split('-')]) + tuple([int(x) for x in AOS[11:].split(':')])
    aos = datetime(*AOS).timestamp()  # Convert tuple.
aos_utc = datetime.fromtimestamp(aos, tz=timezone.utc)
...
try:
    ...
    LOS = tuple([int(x) for x in LOS[:10].split('-')]) + tuple([int(x) for x in LOS[11:].split(':')])
    los = datetime(*LOS).timestamp()  # Convert tuple.
los_utc = datetime.fromtimestamp(los, tz=timezone.utc)
...
# Set colors bases in command line options.
